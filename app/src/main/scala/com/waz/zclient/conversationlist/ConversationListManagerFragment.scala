...
class ConversationListManagerFragment extends Fragment
  ...
  lazy val animationType = {
    ...
    hasConvs.map {
      case true => INFINITE_LOADING_BAR()
      case _    => SPINNER()
    }
  }
  ...
  override def onCreateView(inflater: LayoutInflater, container: ViewGroup, savedInstanceState: Bundle) =
    returning(inflater.inflate(R.layout.fragment_conversation_list_manager, container, false)) { view =>
      ...
      hasConvs.onUi(navController.setPagerEnabled)
  ...
  override def onShowPickUser(destination: Destination) =
    if (getCurrentPickerDestination != destination) {
    ...
    navController.getCurrentLeftPage match {
      ...
      case START | CONVERSATION_LIST=>
        Option(getChildFragmentManager.findFragmentByTag(PickUserFragment.TAG)).foreach {
          case _: PickUserFragment =>
  ...
  override def onPageVisible(page: Page) = {
    import Page._
    if (page != ARCHIVE && page != CONVERSATION_MENU_OVER_CONVERSATION_LIST) closeArchive()
    if (page != CONVERSATION_LIST) navController.setPagerEnabled(false)
  }
  ...
  override def showArchive() = {
    ...
    navController.getCurrentLeftPage match {
      case START | CONVERSATION_LIST =>
        Option(getChildFragmentManager.findFragmentByTag(ArchiveListFragment.TAG)) match {
          case Some(f: ArchiveListFragment) => // already showing
          case None | Some(_) =>
            getChildFragmentManager.beginTransaction
              .setCustomAnimations(
                R.anim.slide_in_from_bottom_pick_user,
...
//    getStoreFactory.conversationStore.addConversationStoreObserverAndUpdate(this) TODO - was this doing anything?
...
//    getStoreFactory.conversationStore.removeConversationStoreObserver(this)
  ...
  override def onOptionsItemClicked(convId: ConvId, user: User, item: OptionsMenuItem) = {
    val userId = new UserId(user.getId)
    ...
    item match {
      ...
      case BLOCK     => showBlockConfirmation(userId)
      case UNBLOCK   => zms.head.flatMap(_.connection.unblockConnection(userId))
